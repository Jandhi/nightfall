...
pub fn update_description(
    ...
    mut q_taken_title: Query<
        (Entity, &mut Text),
        (With<TakenTitle>, Without<Window>, Without<TakenAbility>),
    >,
    mut q_taken_description: Query<
        (Entity, &mut Text),
        (
            With<TakenDescription>,
            Without<Window>,
    ...
    if let Some(cursor_position) = window.cursor_position() {
        ...
        for (transform, taken, collider) in q_takens.iter() {
            ...
            if let (Ok((_, mut title)), Ok((_, mut desc))) = (
                q_taken_title.get_single_mut(),
                q_taken_description.get_single_mut(),
            ) {
