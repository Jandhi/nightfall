...
pub fn projectile_collision_check(
    ...
    for ev_is_colliding in ev_collision.iter() {
        if let (Ok(bullet), Ok((health, member)))
             = (q_projectiles.get_mut(ev_is_colliding.collision.entity_a), q_hittable.get_mut(ev_is_colliding.collision.entity_b)) {
        ...
        if let (Ok(bullet), Ok((health, member)))
             = (q_projectiles.get_mut(ev_is_colliding.collision.entity_b), q_hittable.get_mut(ev_is_colliding.collision.entity_a)) {
            handle_projectile_collision(
                ev_is_colliding.collision.entity_b, bullet,  
                ev_is_colliding.collision.entity_a, 
                health, 
                member.team,
                &mut commands
