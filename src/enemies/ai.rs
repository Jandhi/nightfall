...
impl MoveAndShootAI {
    pub fn new(speed : f32, corrective_force : f32, shoot_distance : f32, charge_time : f32, refresh_time : f32) -> MoveAndShootAI {
        MoveAndShootAI { 
...
pub fn move_and_shoot_ai(
    ...
    for (entity, transform, mut ai, mut velocity) in q_enemies.iter_mut() {
        ...
        if ai.state == MoveAndShootAIState::Move && (player_transform.translation.distance(transform.translation) <= ai.shoot_distance) && ai.refresh_timer.finished() {
